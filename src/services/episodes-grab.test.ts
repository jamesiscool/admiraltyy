...
// Copy types/function from episodes.ts to avoid env.ts import chain
interface GrabEpisodeReleaseInput {
	episodeId: string
	guid: string
	title: string
	downloadUrl: string
...
interface GrabEpisodeReleaseDeps {
...
// Pure function for grabbing an episode release - mirrors episodes.ts logic
async function grabEpisodeReleaseCore(data: GrabEpisodeReleaseInput, deps: GrabEpisodeReleaseDeps) {
...
describe('grabEpisodeReleaseCore', () => {
	let db: TestDb
	let nzbgetServer: StubNzbgetServer
	let deps: GrabEpisodeReleaseDeps
	...
	const fakeNzbPath = '/tmp/test-episode.nzb'
	...
	beforeEach(async () => {
		...
		// Insert test series -> season -> episode hierarchy
		const [series] = db
	...
	const validInput: GrabEpisodeReleaseInput = {
		episodeId: '1',
		guid: 'ep-abc123-guid',
		title: 'Breaking.Bad.S01E01.Pilot.1080p.BluRay.x264',
		downloadUrl: 'https://indexer.example/nzb/ep-abc123',
		infoUrl: 'https://indexer.example/details/ep-abc123',
		size: 1_500_000_000,
	...
	it('downloads NZB and queues to NZBGet', async () => {
		const result = await grabEpisodeReleaseCore(validInput, deps)
		...
		expect(nzbgetServer.calls[0].params[0]).toBe('Breaking.Bad.S01E01.Pilot.1080p.BluRay.x264.nzb')
		expect(nzbgetServer.calls[0].params[2]).toBe('tv')
	...
	it('creates release record in database with episodeId', async () => {
		const result = await grabEpisodeReleaseCore(validInput, deps)
		...
		expect(release.episodeId).toBe(testEpisodeId)
	...
	it('calls notifyActivityFn to trigger fast polling', async () => {
		await grabEpisodeReleaseCore(validInput, deps)

		expect(deps.notifyActivityFn).toHaveBeenCalledOnce()
	})

	it('throws on invalid episode ID format', async () => {
		const invalidInput = { ...validInput, episodeId: 'not-a-number' }

		await expect(grabEpisodeReleaseCore(invalidInput, deps)).rejects.toThrow('Invalid episode ID')
	})
	...
	it('throws when NZBGet returns invalid ID', async () => {
		...
		await expect(grabEpisodeReleaseCore(validInput, deps)).rejects.toThrow('NZBGet failed to queue download')
	})

	it('sanitizes filename for NZBGet', async () => {
		const inputWithSpecialChars: GrabEpisodeReleaseInput = {
			...validInput,
			title: 'Show: The [Special] Edition S01E01 (2023)',
		}

		await grabEpisodeReleaseCore(inputWithSpecialChars, deps)

		// Check the filename passed to NZBGet
		const appendCall = nzbgetServer.calls.find((c) => c.method === 'append')
		expect(appendCall?.params[0]).toBe('Show__The__Special__Edition_S01E01__2023_.nzb')
	})

	it('queues NZB in tv category', async () => {
		await grabEpisodeReleaseCore(validInput, deps)

		const appendCall = nzbgetServer.calls.find((c) => c.method === 'append')
		expect(appendCall?.params[2]).toBe('tv')
	})

	it('handles optional infoUrl being undefined', async () => {
		const inputWithoutInfoUrl = { ...validInput, infoUrl: undefined }

		const result = await grabEpisodeReleaseCore(inputWithoutInfoUrl, deps)
	...
	it('handles multiple grabs for different episodes', async () => {
		...
		// Grab first episode release
		const input1 = { ...validInput, episodeId: '1', guid: 'guid-1', title: 'Breaking.Bad.S01E01.1080p' }
		const result1 = await grabEpisodeReleaseCore(input1, deps)

		// Grab second episode release
		const input2 = { ...validInput, episodeId: '2', guid: 'guid-2', title: 'Breaking.Bad.S01E02.1080p' }
		const result2 = await grabEpisodeReleaseCore(input2, deps)
		...
		// Verify they link to different episodes
		expect(result1.release.episodeId).toBe(1)
		expect(result2.release.episodeId).toBe(2)
	})

	it('handles multiple grabs for same episode', async () => {
		// Grab first release for same episode
		const input1 = { ...validInput, guid: 'guid-1', title: 'Breaking.Bad.S01E01.720p' }
		const result1 = await grabEpisodeReleaseCore(input1, deps)

		// Grab second release for same episode (different quality)
		const input2 = { ...validInput, guid: 'guid-2', title: 'Breaking.Bad.S01E01.1080p' }
		const result2 = await grabEpisodeReleaseCore(input2, deps)

		// Verify both releases exist for same episode
		const releases = db.select().from(schema.releases).all()
		expect(releases).toHaveLength(2)
		expect(releases[0].episodeId).toBe(testEpisodeId)
		expect(releases[1].episodeId).toBe(testEpisodeId)
