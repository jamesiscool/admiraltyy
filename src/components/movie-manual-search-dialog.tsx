...
export function MovieManualSearchDialog({ movieId, movieTitle, movieYear, open, onOpenChange }: MovieManualSearchDialogProps) {
	...
	}, [open, movieId, results, isSearching])
		...
		>
			<DialogContent className="flex max-h-[85vh] w-full max-w-[min(calc(100vw-40px),1200px)]! flex-col overflow-hidden p-0">
				...
				<div className="flex min-h-0 flex-1 flex-col gap-4 px-6 pb-6">
					...
					{!isSearching && results !== null && results.length === 0 && <div className="py-12 text-center text-muted-foreground">No releases found for this movie.</div>}
